...
1. **必须读**:本目录的 `hh-slices/phase-{N}.json`(本批次对应 phase)
2. **必须读**:`days/TEMPLATE.md` + `days/phase-0/00-terminal-basics.md` + `days/phase-2/day041.md`(风格锚)
3. **按需 WebFetch**:具体章节直接抓(不缓存到本地,除非改写需要多次读)
4. **引用规则**:dayNNN.md 的 §9 延伸阅读只列已抓过的本地路径或稳定 URL,**不允许"请参考 X"作为核心教学手段**
5. **版权**:不复制大段原文(>200 字),可改写、可引用概念,可链接
